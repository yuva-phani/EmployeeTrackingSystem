...
object EmployeeTrackingService {
	...
	val configuration = new SparkConf(true).set("spark.cassandra.connection.host", Properties.ipAddress).setMaster(Properties.master)
	...
	val broadCastListOfDatesInString = sc.broadcast(timeInOffice.map { x => DateTimeUtils.strDateTotimeInSeconds(x(1)) }.collect())
		...
		try{
			...
			val avgTimeInOffice = reduceByKeyTest.map(f => f._2)
					...
					avgTimeInOffice.saveToCassandra(Properties.keySpace, Properties.table)
